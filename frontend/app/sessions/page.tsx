...
export default function SessionsPage() {
  ...
  const avgPerSession = sessions.length > 0 ? (totalEvents / sessions.length).toFixed(1) : "0";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-medium text-slate-800">Sessions</h1>
          <p className="mt-1 text-sm text-slate-500">
            Every visitor session, grouped by session ID. Select one to see their journey.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3">
          <SessionJourney sessionId={selectedId} events={events} loading={eventsLoading} />
        </div>
      </div>
    </div>
  );
